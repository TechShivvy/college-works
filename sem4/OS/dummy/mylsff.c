...
void printsrc(char A[],int n);	// prints string in rev
void check(int *A,int n,char **dirs);	// to check arg is a dir(1), file(2) or error(0)
void sort(char** A,int n); 		// sorts 2d char array
char ** ls(const char *dir,char c);	// ls function
void lsR(char *dir, char **fil);	// ls recursive
int green();				// op text colors
int blue();
int cyan();
int reset();
...
int main(int argc, char *arg[])
{
	...
	{
		//creating a copy of argv, sort(argv) didnt work
...
char ** ls(const char *dir,char c)
{
		...
		switch(errno)
		{
			case EACCES:  	printf("ls: cannot open directory '%s': Permission denied\n",dir); 
		    	...
		    	case ENOTDIR: //printf("'%s' is not a directory\n", dir);
						...
						strcpy(dirs[n++],dir);
						file=1;
						//if the passed arg is not a dir, its prolly a file, so directly jump to stats
						goto here;	
					}
		    		      	break;
		}
	...
	{	
		...
		{	
			...
			((sb.st_mode & S_IXUSR) ? green() : reset());
			((S_ISDIR(sb.st_mode)) ? blue() : ((S_ISLNK(sb.st_mode)) ? cyan() : 0));
			printf("%s \n",dirs[i]);	// this alone enof if no colors
	...
	if(!file)
	{
		for(int i=0; i<n; i++)
		{	
			//to form relative path for the files in dir
			strcpy(path,dir);		
			strcat(path,"/");
			strcat(path,dirs[i]);
...
void sort(char** A,int n)			
{
	...
	for(i=0;i<n-1;i++)
	{
		for(j=0;j<n-i-1;j++)
		{
			//s1 corresponds to str1(A[j]) and s2 to str2(A[j+1]) ; y=1 if the swap happens else y=0
			s1=0,s2=0,y=0;	
			
			//if the 1st char is '/', incrementing ptr		
			if(A[j][0]=='/')
			{
				++A[j];		
				s1=1;
			...
			//if the swap didnt happen, decrement the ptrs
			if(!y)				
			{
				if(s1)
					--A[j];
...
//to print the source file name of a symbolic link from the abs path of the src file
