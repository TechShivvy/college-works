...
void FIFO()
{
	printf("\n----------FIFO Page Replacement Alogrithm----------\n");
...
void OPT()
{
	printf("\n----------OPT Page Replacement Alogrithm----------\n");
	count=0;
	int far=INT_MIN,id;					//far-closest far
...
void LRU()
{
	printf("\n----------LRU Page Replacement Alogrithm----------\n");
	count=0;
	int min,id;
	struct node *ptr=pages,*ptr2=mem,*temp;
	while(ptr)
	{
		...
		else
		{
			...
			while(ptr2)
			{
				id=search_imm_left(pages,ptr,ptr2->data);
				if(id<min)
				{
					min=id;
					temp=ptr2;
...
void LFU()
{
	printf("\n----------LFU Page Replacement Alogrithm----------\n");
	count=0;
	int min,ele,ELE;
	struct node *ptr=pages,*ptr2=mem,*temp;
	while(ptr)
			 	...
			 	if(f[ele]==min)		//other eles with same freq
			  	{
					...
					if(search(pages,ele)<search(pages,ELE))	//for FIFO, if new ele comes b4 prev ELE, update
					{
						ELE=ele;
						temp=ptr2;
					}
