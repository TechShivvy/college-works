...
static const char *const yytname[] =
{
  ...
  "OPERATOR", "ID", "NUMBER", "$accept", "program", "statement", "operand", YY_NULLPTR
};
...
{
       5,    11,     1,    13,     6,     1,     7,     8,     4,     0,
      10,    12
};

static const yytype_int8 yycheck[] =
{
       0,    10,     5,    12,     2,     5,     5,     6,     3,    -1,
       4,     4
};
...
yyreduce:
  ...
  switch (yyn)
    {
  case 4: /* statement: ID ASSIGNMENT operand OPERATOR operand  */
#line 44 "part2345.y"
                                                                {	
															if(!isSyntaxCheckingOver) { 
																fprintf(threeAddressCodeFile, "t%d = %s %s %s\n", tempVariableNum, (yyvsp[-2].str), (yyvsp[-1].str), (yyvsp[0].str));
																fprintf(threeAddressCodeFile, "%s = t%d\n", (yyvsp[-4].str), tempVariableNum);
																tempVariableNum++;
															} else if (!isCodeOptimizationOver && isSyntaxCheckingOver) { // code optimisation
																int leftIsNum = isNumber((yyvsp[-2].str));
																int rightIsNum = isNumber((yyvsp[0].str));
																if(leftIsNum && rightIsNum) {
																	// perform constant folding: t0 = 5+7 will be t0 = 12 
																	int left = getNumericalValue((yyvsp[-2].str));
																	int right = getNumericalValue((yyvsp[0].str));
																	int result = doMath(left, right, (yyvsp[-1].str));
																	...
																	if(left == 1  && strcmp((yyvsp[-1].str), "*")==0) {
																		fprintf(optimizedCodeFile, "%s = %s\n", (yyvsp[-4].str), (yyvsp[0].str));
																	} else if (right == 1 && strcmp((yyvsp[-1].str), "*")==0) {
																		fprintf(optimizedCodeFile, "%s = %s\n", (yyvsp[-4].str), (yyvsp[-2].str));
																	} else if (left == 0 && strcmp((yyvsp[-1].str), "*")==0) {
																		fprintf(optimizedCodeFile, "%s = %d\n", (yyvsp[-4].str), 0);
																	} else if (right == 0 && strcmp((yyvsp[-1].str), "*")==0) {
																		fprintf(optimizedCodeFile, "%s = %d\n", (yyvsp[-4].str), 0);
																	} 
																	else if (right == 0 && (strcmp((yyvsp[-1].str), "+")==0 || strcmp((yyvsp[-1].str), "-")==0)) {
																		fprintf(optimizedCodeFile, "%s = %s\n", (yyvsp[-4].str), (yyvsp[-2].str));
																	} else if (left == 0 && (strcmp((yyvsp[-1].str), "+")==0 || strcmp((yyvsp[-1].str), "-")==0)) {
																		fprintf(optimizedCodeFile, "%s = %s\n", (yyvsp[-4].str), (yyvsp[0].str));
																	} else {
...
#line 1213 "y.tab.c"
  ...
  case 5: /* statement: ID ASSIGNMENT operand OPERATOR operand OPERATOR operand  */
#line 106 "part2345.y"
                                                                                        {
																			fprintf(threeAddressCodeFile, "t%d = %s %s %s\n", tempVariableNum, (yyvsp[-4].str), (yyvsp[-3].str), (yyvsp[-2].str));
																			fprintf(threeAddressCodeFile, "t%d = t%d %s %s\n", tempVariableNum+1, tempVariableNum, (yyvsp[-1].str), (yyvsp[0].str));
																			fprintf(threeAddressCodeFile, "%s = t%d\n", (yyvsp[-6].str), tempVariableNum+1);
																			tempVariableNum += 2;
																		}
#line 1224 "y.tab.c"
    break;

  case 6: /* statement: ID ASSIGNMENT operand  */
#line 112 "part2345.y"
                                                        {
											if(!isSyntaxCheckingOver) {
